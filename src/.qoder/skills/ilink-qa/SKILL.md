...
```
❌ 缺少 usage-value 参数。请先在 Qoder 中执行 /usage 查看"已使用 credits"，然后以下列格式重试：

  /ilink-qa <story-id> <已使用 credits 数字>

例如，/usage 显示已使用 1200 credits，则执行：/ilink-qa kcia-1520 1200
...
4. 在表格末尾追加一行：`| review-<N+1> | <时间戳> | <usage-value> | qoder-credits-cumulative |`
5. 计算 Latest Delta：`<本次 usage-value> - <init 行的 Usage_Value>`
   - 若结果为正数 → 更新文件末尾 `**Latest Delta**: <delta> (qoder-credits-cumulative)`
   - 若结果为负数 → 更新为 `**Latest Delta**: (跨 reset 边界, 不可比)`
   - 若 init 或本次 usage-value 之一为 0 → 更新为 `**Latest Delta**: <delta> (含 0 值, 不可信)`
6. Usage_Unit 字段**硬编码** `qoder-credits-cumulative`，不接受用户输入
